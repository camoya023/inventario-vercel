...
async function cargarVistaDetalleVenta(idVenta) {
  ...
  try {
    ...
    if (!data || !data.exito) {
      const mensaje =
        data?.mensaje || "No se encontraron datos para esta venta";
...
function renderizarDatosDetalleVenta(datos) {
  ...
  // Rellenar header
  $("#detalle-codigo-venta").text(
    venta.codigo_venta || `VENTA-${venta.id_venta.substring(0, 8)}`,
  );
  $("#detalle-estado-venta").html(
    `<span class="badge ${estadoClase}">${venta.estado}</span>`,
  );
  $("#detalle-estado-pago-header").html(
    `<span class="badge ${pagoClase}">${venta.estado_pago}</span>`,
  );
  ...
  $("#detalle-cliente-telefono").text(
    cliente ? cliente.telefono_principal : "N/A",
  );
  ...
  if (detalles && detalles.length > 0) {
    ...
    tbodyProductos.append(
      '<tr><td colspan="8" style="text-align:center;">No hay productos en esta venta.</td></tr>',
    );
  }
  ...
  $("#detalle-descuentos").text(
    "-" + formatCurrency(venta.monto_descuento_global || totalDescuentos),
  );
  $("#detalle-impuestos").text(
    formatCurrency(venta.impuestos || totalImpuestos),
  );
  ...
  if (pagos && pagos.length > 0) {
    pagos.forEach((p) => {
      ...
      const fila = `
                <tr>
                    <td>${moment.utc(p.fecha_pago).format("DD/MM/YYYY")}</td>
    ...
    tbodyPagos.append(
      '<tr><td colspan="4" style="text-align:center;">No se han registrado pagos para esta venta.</td></tr>',
    );
  }
